...
#pragma GCC diagnostic pop
    ...
    /* combine results */
    s[7] += s[2] + ctx->__state[1];
    ctx->__state[1] = ctx->__state[2] + s[3] + s[4];
    ctx->__state[2] = ctx->__state[3] + s[0] + s[5];
    ctx->__state[3] = ctx->__state[0] + s[1] + s[6];
    ctx->__state[0] = s[7];
}
...
A_HASH_PROC(a_rmd128_s, a_rmd128_proc, a_rmd128_compress)
