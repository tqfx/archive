...
static void a_whirlpool_compress(a_whirlpool_s *ctx, const unsigned char *buf)
{
    uint64_t k[2][sizeof(ctx->__state) / sizeof(*ctx->__state)];
    uint64_t t[3][sizeof(ctx->__state) / sizeof(*ctx->__state)];
    ...
    for (unsigned int i = 0; i != sizeof(ctx->__state) / sizeof(*ctx->__state); ++i)
    {
        ...
        LOAD64H(t[0][i], buf + sizeof(*ctx->__state) * i);
        t[2][i] = t[0][i];
        t[0][i] ^= k[0][i];
    }
...
#define theta_pi_gamma(a, i) \
    ...
    for (unsigned int i = 0; i != 10; i += 2)
    {
        ...
        for (unsigned int j = 0; j != sizeof(ctx->__state) / sizeof(*ctx->__state); ++j)
        {
...
#undef GB
    ...
    for (unsigned int i = 0; i != sizeof(ctx->__state) / sizeof(*ctx->__state); ++i)
    {
        ctx->__state[i] ^= t[0][i] ^ t[2][i];
    }
}

void a_whirlpool_init(a_whirlpool_s *ctx)
{
    assert(ctx);
...
A_HASH_PROC(a_whirlpool_s, a_whirlpool_proc, a_whirlpool_compress)
