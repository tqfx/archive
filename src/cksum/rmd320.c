...
#define SWAP(t, x, y) (t = x, x = y, y = t)
    ...
    SWAP(t, s[4], s[4 + ((sizeof(s) / sizeof(*s)) >> 1)]);
#undef SWAP
...
void a_rmd320_init(a_rmd320_s *ctx)
{
    ...
    ctx->__state[5] = 0x76543210;
    ctx->__state[6] = 0xFEDCBA98; // 0x01234567 ^ 0xFFFFFFFF
    ctx->__state[7] = 0x89ABCDEF; // 0x76543210 ^ 0xFFFFFFFF
    ctx->__state[8] = 0x01234567;
    ctx->__state[9] = 0x3C2D1E0F; // 0xC3D2E1F0 ^ 0xFFFFFFFF
}

A_HASH_PROC(a_rmd320_s, a_rmd320_proc, a_rmd320_compress)
