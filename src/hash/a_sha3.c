...
static unsigned char *done(a_sha3_s *ctx, void *out, uint64_t pad)
{
...
void a_sha3_224_init(a_sha3_s *ctx)
{
    AASSERT(ctx);
...
void a_sha3_256_init(a_sha3_s *ctx)
{
    AASSERT(ctx);
...
void a_sha3_384_init(a_sha3_s *ctx)
{
    AASSERT(ctx);
...
void a_sha3_512_init(a_sha3_s *ctx)
{
    AASSERT(ctx);
...
int a_sha3_process(a_sha3_s *ctx, const void *pdata, size_t nbyte)
{
    AASSERT(ctx);
    AASSERT(!nbyte || pdata);
    ...
    if (nbyte < old_tail)
    {
        ...
        while (nbyte--)
        {
            ctx->saved |= (uint64_t)(*(p++)) << ((ctx->byte_index++) << 3);
        }
        return A_HASH_SUCCESS;
    }
...
unsigned char *a_sha3_done(a_sha3_s *ctx, void *out)
{
    AASSERT(ctx);
...
unsigned char *a_keccak_done(a_sha3_s *ctx, void *out)
{
    AASSERT(ctx);
...
void a_shake128_init(a_sha3_s *ctx)
{
    AASSERT(ctx);
...
void a_shake256_init(a_sha3_s *ctx)
{
    AASSERT(ctx);
...
unsigned char *a_shake128_done(a_sha3_s *ctx, void *out)
{
    AASSERT(ctx);
...
unsigned char *a_shake256_done(a_sha3_s *ctx, void *out)
{
    AASSERT(ctx);
...
void a_sha3shake_done(a_sha3_s *ctx, unsigned char *out, unsigned int siz)
{
