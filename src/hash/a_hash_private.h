/*!
 @file a_hash_private.h
 @brief hash library private
 @copyright Copyright (C) 2020 tqfx. All rights reserved.
*/

/* Define to prevent recursive inclusion */
#ifndef __A_HASH_PRIVATE_H__
#define __A_HASH_PRIVATE_H__

#include "a_hash.h"

#include <string.h> /* memset, memcpy */

#if defined(_MSC_VER) /* intrinsic */
#include <stdlib.h>
#endif /* _MSC_VER */
...
#define STORE32H(x, y)                       \
    do                                       \
    {                                        \
        uint32_t t__ = __builtin_bswap32(x); \
        memcpy(y, &t__, 4);                  \
    } while (0)
#else
#define LOAD32H(x, y)                               \
...
#define STORE64H(x, y)                       \
    do                                       \
    {                                        \
        uint64_t t__ = __builtin_bswap64(x); \
        memcpy(y, &t__, 8);                  \
    } while (0)
#else
#define LOAD64H(x, y)                                 \
...
#define A_HASH_PROCESS(hash, func, compress)                           \
    ...
    {                                                                  \
        ...
        if (ctx->length + (nbyte << 3) < ctx->length)                  \
        {                                                              \
        ...
        while (nbyte)                                                  \
        {                                                              \
            if ((ctx->cursiz == 0) && (sizeof(ctx->buf) - 1 < nbyte))  \
            {                                                          \
                compress(ctx, p);                                      \
