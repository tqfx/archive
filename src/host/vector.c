...
a_int_t a_vector_copy(a_vector_s *ctx, const a_vector_s *obj)
{
    ...
    a_size_t memory = a_vector_mem_(ctx);
    ctx->__head = malloc(memory);
    if (ctx->__head == a_null)
    {
        return A_FAILURE;
    }
    memcpy(ctx->__head, obj->__head, memory);
    ctx->__last = (a_byte_t *)ctx->__head + number;
    ctx->__tail = (a_byte_t *)ctx->__head + memory;
...
a_vector_s *a_vector_move(a_vector_s *ctx, a_vector_s *obj)
{
    ...
    memset(obj, nil, sizeof(a_vector_s));
    return ctx;
}
...
a_vptr_t a_vector_push(a_vector_s *ctx)
{
    ...
    if (ctx->__last >= ctx->__tail)
    {
        ...
        ctx->__head = head;
        ctx->__last = (a_byte_t *)head + number;
        ctx->__tail = (a_byte_t *)head + memory;
    }
