...
a_avl_s *a_avl_tear(a_avl_u *root, a_avl_s **next)
{
    a_avl_s *node = *next;
    if (!node)
    {
...
static A_INLINE a_void_t a_avl_set_child(a_avl_s *node, a_avl_s *child, const a_int_t sign)
{
    if (sign < 0)
    {
        node->left = child;
...
*/
static a_void_t a_avl_rotate(a_avl_u *root, a_avl_s *A, const a_int_t sign)
{
    a_avl_s *P = a_avl_parent(A);
    a_avl_s *B = a_avl_child(A, -sign);
    a_avl_s *E = a_avl_child(B, +sign);
...
*/
static a_avl_s *a_avl_rotate2(a_avl_u *root, a_avl_s *B, a_avl_s *A, const a_int_t sign)
{
    a_avl_s *P = a_avl_parent(A);
    a_avl_s *E = a_avl_child(B, +sign);
    a_avl_s *F = a_avl_child(E, -sign);
    a_avl_s *G = a_avl_child(E, +sign);
    const a_int_t e = a_avl_factor(E);
...
a_avl_s *a_avl_insert(a_avl_u *root, a_avl_s *node, a_int_t (*cmp)(a_cptr_t, a_cptr_t))
{
...
a_void_t a_avl_remove(a_avl_u *root, a_avl_s *node)
{
    a_avl_s *parent;
