...
static const unsigned int keccakf_piln[24] = {10, 7, 11, 17, 18, 3, 5, 16, 8, 21, 24, 4, 15, 23, 19, 13, 12, 2, 20, 14, 22, 9, 6, 1};
static const unsigned int keccakf_rotc[24] = {1, 3, 6, 10, 15, 21, 28, 36, 45, 55, 2, 14, 27, 41, 56, 8, 25, 43, 62, 18, 39, 61, 20, 44};
...
static unsigned char *a_done(a_sha3_t *ctx, unsigned char *out, uint64_t pad)
{
    ...
    if (out && out != ctx->sb)
    {
        (void)memcpy(out, ctx->sb, ctx->capacity_words << 2);
    }
...
void a_sha3_shake_done(a_sha3_t *ctx, unsigned char *out, unsigned int len)
{
    ...
    if (!ctx->xof_flag)
    {
        ...
        ctx->s[ctx->word_index] ^= (ctx->saved ^ (0x1F << (ctx->byte_index << 3)));
